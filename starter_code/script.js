...
Car.prototype.drawCar = function(){
    ...
    theImage.src = this.img;  // gets the image from the car variable
    ctx.drawImage(theImage, this.x, this.y, this.width, this.height); //getting all the values from the Car constructor function
}

Car.prototype.move = function(magicalNumber){
  ctx.clearRect(this.x, this.y, this.width, this.height);// erase rectangle('car') to give it its new position
  ...
  this.drawCar();// draw the car in its new position depending on key that was pressed
}
...
Obstacle.prototype.draw = function(){
  ...
  document.onkeydown  = function(event){
    ...
    if (event.which === 37 || event.which === 38 || event.which === 39 || event.which40){
